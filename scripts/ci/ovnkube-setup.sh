...
do_create () {
    ...
    curl -Lo $WORKDIR/bin/kind "https://kind.sigs.k8s.io/dl/${KIND_VERSION}/kind-$(uname)-amd64"
    chmod +x $WORKDIR/bin/kind

    curl -Lo ${WORKDIR}/bin/kubectl "https://storage.googleapis.com/kubernetes-release/release/$K8S_VERSION/bin/$OS/$ARCH/kubectl"
...
do_start() {
    ...
    (
        K8S_VERSION=$K8S_VERSION KIND_INSTALL_INGRESS=true PATH="$PATH:${WORKDIR}/bin" ./kind.sh
    )
    popd
}
...
do_stop() {
    if [ -d "$GOPATH/src/github.com/ovn-org/ovn-kubernetes/contrib" ]; then
        pushd "$GOPATH/src/github.com/ovn-org/ovn-kubernetes/contrib"
        (
            K8S_VERSION=$K8S_VERSION KIND_INSTALL_INGRESS=true PATH="$PATH:${WORKDIR}/bin" ./kind.sh --delete
        )
    fi
}
